...
class SupportResistanceAnalyzerPhase1:
    ...
    def identify_technical_levels(self, timeframe: str, symbol: str = 'BTCUSDT') -> Dict[str, List[Dict]]:
        ...
        try:
            ...
            lookback = self.timeframe_config[timeframe]['lookback'] * 3  # 3倍回溯确保足够数据
    ...
    def multi_timeframe_analysis(self, symbol: str = 'BTCUSDT') -> Dict[str, Any]:
        ...
        for timeframe, config in self.timeframe_config.items():
            ...
            # 识别技术位
            technical_levels = self.identify_technical_levels(timeframe, symbol)
            
            # 识别动态位
            dynamic_levels = self.identify_dynamic_levels(timeframe, symbol)
            
            # 合并当前时间框架的位点
            timeframe_supports = technical_levels['supports'] + dynamic_levels['supports']
            timeframe_resistances = technical_levels['resistances'] + dynamic_levels['resistances']
        ...
        # 7. 过滤弱位点（评分<4）
        strong_supports = [s for s in scored_supports if s.get('final_score', 0) >= 4]
        strong_resistances = [r for r in scored_resistances if r.get('final_score', 0) >= 4]
        
        logger.info(f"分析完成: 找到 {len(strong_supports)} 个强支撑位和 {len(strong_resistances)} 个强阻力位")
    ...
    def calculate_strength_score(self, level: Dict, level_type: str, current_price: float) -> Dict:
        ...
        # 1. 时间框架权威性
        timeframe_score = self._calculate_timeframe_score(level)
        score += timeframe_score
        score_details['timeframe_score'] = timeframe_score
        
        # 2. 触及测试次数（简化版，使用source_count）
        touch_score = min(level.get('source_count', 1), 4)
        score += touch_score
        score_details['touch_score'] = touch_score
        
        # 3. 技术位重合度
        confluence_score = self._calculate_confluence_score(level)
        score += confluence_score
        score_details['confluence_score'] = confluence_score
        
        # 4. 动态位重合度
        dynamic_score = self._calculate_dynamic_score(level)
        score += dynamic_score
        score_details['dynamic_score'] = dynamic_score
        
        # 5. 心理位重合
        psychological_score = self._calculate_psychological_score(level)
        score += psychological_score
        score_details['psychological_score'] = psychological_score
        
        # 6. 近期有效性（简化版，检查是否接近当前价格）
        recent_score = self._calculate_recent_score(level, current_price)
        score += recent_score
        score_details['recent_score'] = recent_score
        
        # 确保分数在1-15范围内
        final_score = max(1, min(score, 15))
        ...
        level['score_details'] = score_details
    ...
    def generate_report(self, analysis_result: Dict) -> str:
        ...
        report = []
        report.append("=" * 80)
        report.append(f"支撑阻力分析报告 - {symbol}")
